...
**Branch:** `main`
**Commit de referência:** `f46378f`
**Working tree:** clean após a otimização do fluxo frontend-backend e a atualização documental desta revisão
...
| Retenção de raws | 97 raws arquivados byte a byte, 1.189.422 bytes, 0 divergências SHA-256 na última indexação; inclui baseline e relatório de sincronização frontend. | `evidence/raw-evidence-manifest.json`, `docs/raw-evidence-index.md` |
...
| P2-API-001 | API | Extrair políticas MCP testáveis e reduzir acoplamento do registro de processos sem breaking change. | Concluída | P0-MCP-001, P1-SEC-003 | Testes puros de read/write/unknown/malformed, deny-by-default para tool desconhecida e contexto nomeado em `server.ts`; comandos existentes preservados. |
